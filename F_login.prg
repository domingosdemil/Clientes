		...
		*** Caso o Unico Usuário cadastrado seja o Usuário Padrão, Mostra uma mensagem para o usuário.
		If AllTrim(Acesso->APELIDO) == "USUARIO" .And. Decripta( Acesso->Senha ) == "SENHA"
			...
			**** Muda Status do Label Mensagens (F_MENU.PRG) para Visivel
			Principal.Label_Mensagens.Visible := .T.	
			
			**** Coloca a Mensagem no Valor do Label			 
			Principal.Label_Mensagens.value := "Se você ainda não cadastrou nenhum usuário,  Digite:  USUARIO no campo  USUARIO   e   SENHA no campo  SENHA"
	 	 ...
	 	 @045,030 LABEL Label_Password	;
			  ...
			  ON ENTER  Iif( ! Empty( Form_acesso.p_password.Value ) ,  Form_acesso.Bt_Login.SetFocus , Form_acesso.p_password.SetFocus )
                             ...
                             @ 090,143 BUTTON Bt_Logoff                   ;
                                             CAPTION '&Cancela'                 ;
                                             ACTION Principal.Release           ;
                                            FONT "MS Sans Serif" SIZE 09 FLAT

	END WINDOW

	*** Coloca o Cursor/Foco  no TEXTBOX p_user
 ...
 Local aStatusDoUsuario := {}	
	...
	*** Se o Apelido digitado em TextBox p_User for encontrado
	If Acesso->(DBSeek( cUser ))	
		...
		*** Decriptografa a Senha do usuário armazenada no arquivo e compara com a senha digitada		
		If Decripta( Acesso->Senha ) != cPass
			...
			*** Se for diferente, envia mensagem e posiciona o cursor / Foco  no campos p_password
	...
	Else
		...
		** Se o usuário/Apelido não existir, emite mensagem e posiciona o cursor / Foco em p_User
	...
	*** Se Usuário estiver Inativo, envia mensagem para o usuário, limpa os campos do formulario  e posiicona o cursor / Foco em p_User 
  ...
  EndIf  
	...
	*** Muda o Status do Menu Usuários para Habilitado
	MODIFY CONTROL Mn_Usuarios OF Principal ENABLED .T.
	MODIFY CONTROL Mn_Indexar OF Principal ENABLED .T.
  ...
  if aStatusDoUsuario[ 1 ] != "0"    && Somente usuários de Nível "0" (Zero) Podem acessar cadastro de Usuários
		...
		*** Se usuário atual não tem Nivel 0 (Zero) Desabilita o menu Usuários
		MODIFY CONTROL Mn_Usuarios OF Principal ENABLED .F.	
    MODIFY CONTROL Mn_Indexar OF Principal ENABLED .F.

		*** Se a data de validade é menor que a Data atual do sistema,  envia mensagem para o usuário, limpa os campos do formulario  e posiicona o cursor / Foco em p_User 
  ...
  Form_acesso.Release
	...
	*** Muda Status da linha de mensagens para Invisivel	
	Principal.Label_Mensagens.Visible := .F.	
          
          	*** Coloca o Apelido do usuário atual na linha de Status do Menu
	Principal.StatusBar.Item(3) := Acesso->Apelido
...
FUNCTION AcessoOpen()
 ...
 If nArea == 0	     
		...
		** Se Não existir o arquivo	
		If ! FILE( ArqBase )			
			...
			*** Quando o arquivo é criado, cria-se também um usuário Padrão
			...
			ACESSO->STATUS  := Encripta( '011110'+DtoC( "31/12/2050") ) && o usuário padrão terá validade permanente
		...
		*** Se não existir o Arquivo Acesso.CDX, cria.
		If ! File( cBase+'ACESSO.CDX' )
			Index ON CODIGO  TAG CODIGO  To (cBase)+"Acesso"
			Index ON APELIDO TAG APELIDO To (cBase)+"Acesso"
		EndIf
